...
/// Обёртка над `index_codebase` для тестов.
///
/// Реестр блокировок и кэш поиска — не предмет этих тестов, но у функции они
/// обязательные аргументы. Реестр держим ОДИН на тест-бинарь (как на живом
/// сервере, где он живёт со стартом процесса): свежий реестр на каждый вызов
/// молча снимал бы сериализацию одинаковых воркспейсов, то есть тесты гоняли бы
/// не тот путь, что прод. Кэш поиска — `None`: тесты его не наблюдают.
